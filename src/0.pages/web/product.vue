  ...
  <div class="page__product">
    <div class="page__product_item">
      <div class="page__product_item--picture">
        ...
        <i18n-n v-if="product.promo" class="page__product_item--picture-promo" :value="parseFloat(product.promo)" format="percent" />
        <i18n-n :class="['page__product_item--picture-price', product.promo ? 'old__price' : '']" :value="parseFloat(product.price)" format="currency" />
        <i18n-n v-if="product.promo" :class="['page__product_item--picture-price', product.promo ? 'new__price' : '']" :value="parseFloat(product.price * ( 1 - product.promo ))" format="currency" />
      </div>
      <div class="page__product_item--description">
        <div class="page__product_item--description-options">
          ...
          >
            {{$t('pages.shop.details')}}
          </div>
        </div>
        <div class="page__product_item--description-content">
          <div class="content__body" v-show="description === 'description'" v-html="product.description">
          </div>
          <div class="content__body" v-show="description === 'details'">
            <div
              v-for="(d, di, dk) in product.details"
              :key="dk" class="content__body_item"
            >
              {{d}}
            </div>
          </div>
    ...
    <div class="page__product_actions">
      <router-link to="/shop" class="page__product_actions--action link"  :title="$t('navigate.return')">
        <div class="page__product_actions--action-icon">
          <Icons icon="ArrowLeft" class="icon"/>
        </div>
        <span class="page__product_actions--action-label">
          {{$t('navigate.return')}}
        </span>
      </router-link>
      <PluginCounter @getValue="counterValue" class="page__product_actions--action counter"/>
      <div class="page__product_actions--action-icon counter__add"
        :title="$t('pages.shop.choose')"
        @click="addToCart"
      >
        <Icons icon="CartAdd" :title="$t('pages.shop.choose')" class="icon" />
      </div>
      <div
        ...
        @click="goToPurchase"
      >
        <Icons :icon="counter > 0 ? 'CartFull' : 'CartVoid'" :title="$t('pages.shop.choose')" class="icon"/>
  ...
  created () {
    ...
    if (this.$route.params.id === undefined) {
      ...
      this.product = products.filter(
        p => this.productID === p.id.toString()
      )[0]
    }
  },
  mounted () {
